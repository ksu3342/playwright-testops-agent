...
def _draft_test_plan_node(tracer: AgentRunTracer):
    def draft_test_plan_node(state: AgentGraphState) -> dict[str, Any]:
        ...
        test_plan = tracer.call_tool(
            "draft_test_plan",
            {"input_path": input_path},
            lambda: tools.draft_test_plan(input_path, testing_context=retrieval_result),
        )
...
def _report_approval_node(tracer: AgentRunTracer):
    def report_approval_node(state: AgentGraphState) -> dict[str, Any]:
        ...
        if decision == "approved":
            return {"pending_approval": None}
        if decision == "rejected":
            next_state = {**state, "final_status": "report_rejected", "pending_approval": None}
            return {
        ...
        next_state = {**state, "final_status": "waiting_for_report_approval", "pending_approval": request}
        return {
